...
export default function EditQuestionPage() {
  ...
  useEffect(() => {
    async function fetchQuestion() {
      try {
        const data = await getQuestionById(game_id)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateQuestion(game_id, { question, answers })
  ...
  if (isLoading) return <div className="text-white text-center p-10">Memuat...</div>
  ...
  return (
    ...
    >
      <div className="relative mx-auto max-w-3xl w-full bg-gradient-to-r from-gray-800 via-gray-700 to-gray-600 text-white p-6 rounded-lg shadow-lg">
        <h2 className="text-2xl font-bold mb-4 text-center">Edit Question</h2>
            ...
            <div className="text-right">
              <div
                className={`inline-block font-bold px-4 py-2 rounded shadow ${
                  totalPoints > 100 ? 'bg-red-400 text-white' : 'bg-yellow-300 text-gray-900'
                }`}
              >
